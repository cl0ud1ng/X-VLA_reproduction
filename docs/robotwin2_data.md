...
当前正式训练参数为：8 卡 DDP、每卡物理 batch `32`（global batch `256`）、`gradient_accumulation_steps=1`、`15,000` optimizer steps、`learning_rate=5e-5`、`freeze_steps=500`、`warmup_steps=1000`、cosine decay、`save_interval=7500`。`5e-5` 是从原 global batch 8 / `1e-5` 按平方根规则得到的保守缩放。batch-256 的 full weight-finetuning smoke 尚待执行，建议使用 `train.py --disable_checkpoint --iters 1` 的命令先做显存验证。
