...
当前正式训练参数为：8 卡 DDP、每卡物理 batch `8`（global batch `64`）、`gradient_accumulation_steps=1`、`30,000` optimizer steps、`learning_rate=3e-5`、`freeze_steps=1000`、`warmup_steps=2000`、cosine decay、`save_interval=10000`。`3e-5` 是从原 global batch 8 / `1e-5` 按平方根规则得到的保守缩放。正式 launcher 同时使用 W&B 记录训练曲线和过程参数。此前在 `ssh -p 42983` 的 8×4090 节点测试 per-GPU batch 32 和 16 的 full weight-finetuning 均 OOM；batch 16 在 vision attention 阶段还需额外约 244 MB。batch 8 的 smoke 尚未执行，正式训练启动前应先验证该配置。
