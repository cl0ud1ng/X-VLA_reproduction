...
需要 30,000 optimizer steps 时使用 [`scripts/run_robotwin2_training_8GPU_30k.sh`](../scripts/run_robotwin2_training_8GPU_30k.sh)。该变体固定默认 `seed=1`，每 `10,000` 步保存一次含 training state 的 checkpoint；保持 batch、学习率和其余 FSDP 参数不变，并将 warmup 从 `1,000` 按比例调整为 `2,000` 步。输出目录为 `outputs/robotwin_ft/domain_balanced/seed1`。
