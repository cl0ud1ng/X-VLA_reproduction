...
FSDP/full-FT 可行性 smoke 已完成（节点 `ssh -p 41844`，8×RTX 4090）：Accelerate FSDP1 `FULL_SHARD`、`transformer_based_wrap`、FSDP activation checkpointing、fp16；每卡**物理 batch 32**、`gradient_accumulation_steps=1`，因此 global batch `8×32=256`，没有 micro-batch 或梯度累积。运行 2 个 optimizer step，879,482,456/879,482,456 参数均可训练，loss 从 `217.1363` 降到 `176.9552`；峰值显存为 allocated `13.72 GiB`、reserved `14.60 GiB`（24 GiB 卡），总耗时 40.0 s，状态为 pass。完整命令参数和峰值记录见 [`outputs/robotwin_ft/fsdp_fullft_batch256_smoke/report.json`](../outputs/robotwin_ft/fsdp_fullft_batch256_smoke/report.json)，逐步日志见同目录 `train.log`。测试使用当前提交 `525b67e`，manifest 为总 manifest。

当前正式训练入口 [`scripts/run_robotwin2_training_8x4090.sh`](../scripts/run_robotwin2_training_8x4090.sh) 已切换为 FSDP full fine-tuning：每卡物理 batch `32`、global batch `256`、`gradient_accumulation_steps=1`、`15,000` optimizer steps、`learning_rate=5e-5`、`freeze_steps=0`、`warmup_steps=1000`、cosine decay、`save_interval=7500`。相较 7,500-step 试跑配置，步数和 warmup 同比例扩大；global batch 不变，因此学习率不变。FSDP full-state checkpoint 保存和 `state.json` 会记录 backend、wrap policy、全量可训练参数数目及 global batch。此前 DDP 下 per-GPU batch 32/16 的 full weight-finetuning OOM，不能作为 FSDP 可行性的结论；正式长跑前仍应先重跑上述 smoke 并确认节点空闲显存。
